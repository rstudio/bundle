...
As an example, let's fit a model with the keras package, building a neural network that models handwritten digits from the MNIST data. This code is derived from `vignette("index", package = "keras")`---see that vignette for more explanation about this model fit!
...
Given this new understanding, we can update our mental map a bit. Some objects require extra information when they're loaded into new environments in order to do their thing. In this case, this keras model object needs access to additional reference in order to predict on new data.
...
keras' vignette is really informative in telling us what we ought to do from here---if we save the model with their native serialization rather than `saveRDS`, we'll be good to go.
